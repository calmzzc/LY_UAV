...
def env_agent_config(cfg, seed=1):
    ...
    state_dim = 6 * (env.numberOfSphere + env.numberOfCylinder + env.numberOfCone) + 0
    ...
    for i_ep in range(cfg.train_eps):
        ...
        for i_step in range(cfg.max_step):
            ...
            if i_ep > 100:
                ...
                # for k in range(env.numberOfSphere):
                #     action_sphere = [random.uniform(env.act_bound[0],env.act_bound[1])]
                # for k in range(env.numberOfCylinder):
                #     action_cylinder =[random.uniform(env.act_bound[0],env.act_bound[1])]
                # for k in range(env.numberOfCone):
                #     action_cone = [random.uniform(env.act_bound[0],env.act_bound[1])]
...
if __name__ == '__main__':
    ...
    save_results(rewards, ma_rewards, tag='train_task_' + env.env_name, path=cfg.result_path)
    plot_rewards_cn(rewards, ma_rewards, tag="train_task_" + env.env_name, env=cfg.env, algo=cfg.algo,
                    path=cfg.result_path)
    # 测试
    env, agent = env_agent_config(cfg, seed=1)
    agent.load(path=cfg.model_path)
    rewards, ma_rewards = eval(cfg, env, agent, 'final')
    save_results(rewards, ma_rewards, tag='eval_task_' + env.env_name, path=cfg.result_path)
    plot_rewards_cn(rewards, ma_rewards, tag="eval_task_" + env.env_name, env=cfg.env, algo=cfg.algo,
                    path=cfg.result_path)
    # agent.load_mid(path=cfg.model_path)
    # rewards, ma_rewards = eval(cfg, env, agent, 'mid')
    # save_results(rewards, ma_rewards, tag='mid_eval_task', path=cfg.result_path)
    # plot_rewards_cn(rewards, ma_rewards, tag="mid_eval_task", env=cfg.env, algo=cfg.algo, path=cfg.result_path)
