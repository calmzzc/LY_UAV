...
class APF:
    ...
    def repulsion(self, q, eta):  # 这个版本的斥力计算函数计算的是斥力因子都相同情况下的斥力 2020.12.24被替换
        ...
        for i in range(self.obstacle.shape[0]):  # 球的斥力
            ...
            if r <= self.r0:
                tempfvec = eta * (1 / r - 1 / self.r0) * Rq2qgoal ** 2 / r ** 2 * self.differential(q,
                                                                                                    self.obstacle[i, :]) \
    ...
    def dynamicRepulsion(self, q):  # 动态障碍物的斥力
        ...
        if r <= self.dynamicSpherer0:
            tempfvec = self.dynamicSphereEta * (
                    1 / r - 1 / self.dynamicSpherer0) * Rq2qgoal ** 2 / r ** 2 * self.differential(q,
                                                                                                   self.dynamicSphereXYZ) \
                       + self.dynamicSphereEta * (1 / r - 1 / self.dynamicSpherer0) ** 2 * Rq2qgoal * self.differential(
                q, self.qgoal)
            f0 = f0 + tempfvec
        else:
            tempfvec = np.array([0, 0, 0])
    ...
    def getqNext(self, epsilon, eta1List, eta2List, eta3List, q,
                 qBefore):  # eta和epsilon需要外部提供，eta1List为球的斥力列表，eta2List为圆柱体的斥力表 fix:2021.2.9
        """
    ...
    def getqNext2(self, epsilon, action, q,
                  qBefore):  # eta和epsilon需要外部提供，eta1List为球的斥力列表，eta2List为圆柱体的斥力表 fix:2022.4.28  此种方法由无人机主动探索
        """
        当qBefore为[None, None, None]时，意味着q是航迹的起始点，下一位置不需要做运动学约束，否则进行运动学约束
        """
        Attraction = self.attraction(q, epsilon)  # 计算引力
        action = action * 2 + Attraction
        qBefore = np.array(qBefore)
        if qBefore[0] is None:
            qNext = q + self.stepSize * action  # 计算下一位置
        else:
            qNext = q + self.stepSize * action  # 计算下一位置
            # _, _, _, _, qNext = self.kinematicConstrant(q, qBefore, qNext)
        # self.path = np.vstack((self.path, qNext))  # 记录轨迹
    ...
    def getUnitCompositeForce(self, q, eta1List, eta2List, eta3List, epsilon):
        # 可以修改为引力也用神经网络输出
        # 可以对比神经网络输出UAV个方向加速度以及输出引力斥力的效果
        # 在使用DDPG输出APF斥力时应该考虑引力的问题，引力是否应该由神经网络输出，同时每一位置下引力是否应作为状态输入给神经网络
        Attraction = self.attraction(q, epsilon)  # 计算引力
        ...
        for i in range(len(eta3List)):
            Repulsion = Repulsion + self.repulsionForOneObstacle(q, eta3List[i],
                                                                 np.hstack((self.cone[i, :], self.coneH[i] / 2)))
    ...
    def checkAction(self, q, flag, action, q_before):
        limit = np.array([0, 0, 0])
        action_x = np.array([action[0], 0, 0])
        action_y = np.array([0, action[1], 0])
        action_z = np.array([0, 0, action[2]])
        q_next_x = self.getqNext2(self.epsilon0, action_x, q, q_before)
        q_next_y = self.getqNext2(self.epsilon0, action_y, q, q_before)
        q_next_z = self.getqNext2(self.epsilon0, action_z, q, q_before)
        flag_x = self.checkCollision(q_next_x)
        flag_y = self.checkCollision(q_next_y)
        flag_z = self.checkCollision(q_next_z)
        if (flag_x != np.array([1, -1, -1])).all():
            limit[0] = 1
        if (flag_y != np.array([1, -1, -1])).all():
            limit[1] = 1
        if (flag_z != np.array([1, -1, -1])).all():
            limit[2] = 1
        return limit
        # if flag[1] == 0:  # 只有完全在障碍物内部和在障碍物边界两种情况
        #     dis = self.dCost(q, self.obstacle[flag[2]])
        #     if dis == self.Robstacle[flag[2]]:
        #         index = random.uniform(0, 2)
        #         limit[index] = 1
        #     elif dis < self.Robstacle[flag[2]]:
        #         limit = np.array([1, 1, 1])
        #     return limit
        # elif flag[1] == 1:
        #     dis = self.dCost(q[0:2], self.cylinder[flag[2]])
        #     if dis == self.cylinderR[flag[2]] and 0 <= q[2] <= self.cylinderH[flag[2]]:
        #         index = random.uniform(0, 1)
        #         limit[index] = 1
        #     elif dis < self.cylinderR[flag[2]] and 0 <= q[2] <= self.cylinderH[flag[2]]:
        #         limit = np.array([1, 1, 0])
        #     return limit
        # elif flag[1] == 2:
        #     dis = self.dCost(q[0:2], self.cone[flag[2]])
        #     if 0 <= q[2] and dis == (self.coneR[flag[2]] - q[2] * self.coneR[flag[2]] / self.coneH[flag[2]]):
        #         index = random.uniform(0, 1)
        #         limit[index] = 1
        #     elif 0 <= q[2] and dis < (self.coneR[flag[2]] - q[2] * self.coneR[flag[2]] / self.coneH[flag[2]]):
        #         limit = np.array([1, 1, 0])
        #     return limit
    ...
    def checkCollision(self, q):
        ...
        for i in range(self.numberOfSphere):  # 球的检测碰撞
            if self.distanceCost(q, self.obstacle[i, :]) - 1 <= self.Robstacle[i]:
                return np.array([0, 0, i])
        for i in range(self.numberOfCylinder):  # 圆柱体的检测碰撞
            if 0 <= q[2] <= self.cylinderH[i] and self.distanceCost(q[0:2], self.cylinder[i, :]) - 1 <= self.cylinderR[
                i]:
                return np.array([0, 1, i])
        for i in range(self.numberOfCone):
            if q[2] >= 0 and self.distanceCost(q[0:2], self.cone[i, :]) - 1 <= self.coneR[i] - q[2] * self.coneR[i] / \
                    self.coneH[i]:
                return np.array([0, 2, i])
        return np.array([1, -1, -1])  # 不撞的编码

    @staticmethod
    def dCost(point1, point2):  # 计算两点之间各个坐标轴的距离
    ...
    def saveCSV(self):  # 保存数据方便matlab绘图(受不了matplotlib三维了)
        ...
        np.savetxt(self.time + 'pathMatrix.csv', self.path, delimiter=',')
        np.savetxt(self.time + 'obstacleMatrix.csv', self.obstacle, delimiter=',')
        np.savetxt(self.time + 'RobstacleMatrix.csv', self.Robstacle, delimiter=',')
        np.savetxt(self.time + 'cylinderMatrix.csv', self.cylinder, delimiter=',')
        np.savetxt(self.time + 'cylinderRMatrix.csv', self.cylinderR, delimiter=',')
        np.savetxt(self.time + 'cylinderHMatrix.csv', self.cylinderH, delimiter=',')
        np.savetxt(self.time + 'coneMatrix.csv', self.cone, delimiter=',')
        np.savetxt(self.time + 'coneRMatrix.csv', self.coneR, delimiter=',')
        np.savetxt(self.time + 'coneHMatrix.csv', self.coneH, delimiter=',')

        np.savetxt(self.time + 'start.csv', self.x0, delimiter=',')
        np.savetxt(self.time + 'goal.csv', self.qgoal, delimiter=',')

    def drawPath(self, path):  # 绘制path变量
        fig = plt.figure()
        self.ax = Axes3D(fig)
        self.ax.plot3D(self.path[:, 0], self.path[:, 1], self.path[:, 2], color="deeppink", linewidth=2,
                       label='UAV path')
        plt.savefig(path + "Final_Path")
        plt.show()

    def drawPathmid(self, path):  # 绘制path变量
        fig = plt.figure()
        self.ax = Axes3D(fig)
        self.ax.plot3D(self.path[:, 0], self.path[:, 1], self.path[:, 2], color="deeppink", linewidth=2,
                       label='UAV mid path')
        plt.savefig(path + "Mid_Path")
        plt.show()
